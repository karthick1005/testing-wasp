...
// 📊 TYPES: Import User entity type
import { type User } from 'wasp/entities';
// 🔐 AUTHENTICATION: Import logout function
import { logout } from 'wasp/client/auth';
// 🎭 ICONS: Import React icons for menu items
import { MdOutlineSpaceDashboard } from 'react-icons/md'; // Dashboard icon
import { TfiDashboard } from 'react-icons/tfi'; // Alternative dashboard icon
// 🎨 STYLING: Import conditional CSS class utility
import { cn } from '../client/cn';

/**
 * 👤 USER MENU ITEMS COMPONENT: Navigation menu for authenticated users
 * 🔧 TEMPLATE USAGE: User account and app navigation in mobile menu and dropdowns
 * 
 * Key features:
...
export const UserMenuItems = ({ user, setMobileMenuOpen }: { 
  user?: Partial<User>; // 🔧 Optional user data for personalization
  setMobileMenuOpen?: any // 🔧 Mobile menu state handler (optional)
}) => {
  // 📍 PATH DETECTION: Get current page location for conditional rendering
  const path = window.location.pathname;
  const landingPagePath = routes.LandingPageRoute.to; // 🔧 CHANGE: Update if landing route changes
  const adminDashboardPath = routes.AdminRoute.to; // 🔧 CHANGE: Update if admin route changes

  /**
   * 📱 MOBILE MENU HANDLER: Close mobile menu when item is clicked
   * 🔧 TEMPLATE USAGE: Improves mobile UX by closing menu after navigation
   */
  ...
  return (
    <>
      {/* 📋 MENU ITEMS LIST: Main navigation list for authenticated users */}
      <ul
        className={cn('flex flex-col gap-5 border-b border-stroke py-4 dark:border-strokedark', {
          // 🎨 CONDITIONAL STYLING: Different padding based on current page
          'sm:px-6': path !== adminDashboardPath, // Standard padding for non-admin pages
          'px-6': path === adminDashboardPath, // Admin page padding
          // 🔧 CHANGE: Customize spacing and border styling
        })}
      >
        {/* 🎯 CONDITIONAL DEMO LINK: Show demo app link on landing and admin pages */}
        {path === landingPagePath || path === adminDashboardPath ? (
          <li>
            <WaspRouterLink
              to={routes.DemoAppRoute.to} // 🔧 CHANGE: Update demo app route if different
              ...
              // - gap-3.5: space between icon and text
              // - text-sm: font size
              // - font-medium: font weight
              // - duration-300 ease-in-out: smooth hover transition
              // - hover:text-yellow-500: hover color effect
            >
              {/* 🎨 DEMO ICON: Dashboard icon for demo app */}
              <MdOutlineSpaceDashboard size='1.1rem' />
              {/* 🔧 CHANGE: Replace icon or adjust size */}
              
              {/* 📝 DEMO LINK TEXT: Demo app navigation text */}
              AI Scheduler (Demo App)
              {/* 🔧 CHANGE: Replace with your demo app name */}
            </WaspRouterLink>
          </li>
        ) : null}
        
        {/* 👤 ACCOUNT LINK: User account management page */}
        <li>
          <WaspRouterLink
            to={routes.AccountRoute.to} // 🔧 CHANGE: Update account route if different
            onClick={handleMobileMenuClick} // Close mobile menu on click
            className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'
            // 🔧 CHANGE: Same styling options as demo link above
          >
            {/* 👤 ACCOUNT ICON: SVG icon for account/profile */}
            <svg
              className='fill-current' // Uses current text color
              width='18' // 🔧 CHANGE: Adjust icon width
              height='18' // 🔧 CHANGE: Adjust icon height
              viewBox='0 0 22 22'
              fill='none'
            ...
            >
              <path
                ...
                fill=''
                // 🔧 CHANGE: Settings icon SVG path - replace entire SVG block with your preferred icon
              />
              <path
                ...
                fill=''
                // 🔧 CHANGE: Settings gear center circle SVG path
              />
            </svg>
            {/* 📝 ACCOUNT LINK TEXT: User account page navigation text */}
            Account Settings
            {/* 🔧 CHANGE: Replace with your account page label */}
          </WaspRouterLink>
        </li>
      </ul>
      
      {/* 👑 ADMIN MENU SECTION: Admin-only navigation items with conditional rendering */}
      {!!user && user.isAdmin && (
        // 🔐 ADMIN CHECK: Only show if user exists and has admin privileges
        // 🔧 CHANGE: Modify admin permission logic if using different role system
        <ul
          className={cn('flex flex-col gap-5 border-b border-stroke py-4 dark:border-strokedark', {
            // 🎨 RESPONSIVE PADDING: Conditional spacing based on current page
            'sm:px-6': path !== adminDashboardPath, // Standard responsive padding
            'px-6': path === adminDashboardPath, // Full padding on admin pages
            // 🔧 CHANGE: Customize admin section spacing and borders
          })}
        >
          <li className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'>
            {/* 📊 ADMIN DASHBOARD LINK: Admin panel navigation */}
            <WaspRouterLink
              to={routes.AdminRoute.to} // 🔧 CHANGE: Update admin route if different
              onClick={handleMobileMenuClick} // Close mobile menu on click
              className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500'
              // 🔧 CHANGE: Customize admin link styling (same options as above)
            >
              {/* 🎨 ADMIN ICON: Dashboard icon for admin panel */}
              <TfiDashboard size='1.1rem' />
              {/* 🔧 CHANGE: Replace with different admin icon if preferred */}
              
              {/* 📝 ADMIN LINK TEXT: Admin dashboard navigation text */}
              Admin Dashboard
              {/* 🔧 CHANGE: Replace with your admin panel name */}
            </WaspRouterLink>
          </li>
        </ul>
      )}
      ...
      <button
        ...
        // 🔧 CHANGE: Add custom logout logic here if needed (e.g., analytics, cleanup)
        className={cn(
          'flex items-center gap-3.5 py-4 text-sm font-medium duration-300 ease-in-out hover:text-yellow-500',
          {
            // 🎨 RESPONSIVE SPACING: Conditional padding based on current page
            'sm:px-6': path !== adminDashboardPath, // Standard responsive padding
            'px-6': path === adminDashboardPath, // Full padding on admin pages
            // 🔧 CHANGE: Customize logout button spacing and styling
          }
        )}
      >
        ...
        >
          <path
            ...
            fill=''
            // 🔧 CHANGE: Logout door/exit icon SVG path - replace with preferred logout icon
          />
          <path
            d='M6.05001 11.7563H12.2031C12.6156 11.7563 12.9594 11.4125 12.9594 11C12.9594 10.5875 12.6156 10.2438 12.2031 10.2438H6.08439L8.21564 8.07813C8.52501 7.76875 8.52501 7.2875 8.21564 6.97812C7.90626 6.66875 7.42501 6.66875 7.11564 6.97812L3.67814 10.4844C3.36876 10.7938 3.36876 11.275 3.67814 11.5844L7.11564 15.0906C7.25314 15.2281 7.45939 15.3312 7.66564 15.3312C7.87189 15.3312 8.04376 15.2625 8.21564 15.125C8.52501 14.8156 8.52501 14.3344 8.21564 14.025L6.05001 11.7563Z'
            fill=''
            // 🔧 CHANGE: Logout arrow icon SVG path
          />
        </svg>
        
        {/* 📝 LOGOUT BUTTON TEXT: Logout action text */}
        Log Out
        {/* 🔧 CHANGE: Replace with your logout button label */}
      </button>
    </>
  );
};
